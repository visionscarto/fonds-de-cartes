var visionscarto_bertin_1953_alpha1 = function (λ, φ) {
	...
	if (c.southamerica) {
		bar = 1.3;
	    if (d > bar && λ < -0.75 && theta < -0.22) {
	    	jump += -0.42 * (d - bar);
	    	if (c.shrinksouthamerica) {
		    	d *= (1 - 0.07 * (d - bar));
		    }
		}
	}

	// compensation régionale Océanie
	if (c.oceania) {
		bar = 1.41;
	    if (λ > 1.1 && theta > 0.30 && d > bar) {
	    	jump += 0.53 * (d - bar);
	    }
	}
	...
	if (c.southasia) {
		...
		bar = Math.PI * 110 / 180;
		if (d > 0 && λ > bar) {
			d += factor * 0.22 * Math.pow(λ - bar, 1.5)
		}
	}
